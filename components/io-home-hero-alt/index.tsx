...
export default function IoHomeHeroAlt({ brand, heading, description, ctas, }: IoHomeHeroAltProps) {
    return (<header className={s.hero}>
      <div className={s.patterns}>
        <div className={s.patternsStart}>
          <Image src={require('./pattern-start.svg')} width={418} height={543} layout="fill" objectFit="cover" alt="" priority/>
        </div>
        <div className={s.patternsEnd}>
          <Image src={require('./pattern-end.svg')} width={418} height={543} layout="fill" objectFit="cover" alt="" priority/>
        </div>
      </div>
      <div className={s.container}>
        <div className={s.inner}>
          <Intro textAlignment="center" heading={heading} headingSize={1} description={description} actions={{
            layout: 'stacked',
            theme: brand,
            ctas: ctas.map((cta: {
                title: string;
