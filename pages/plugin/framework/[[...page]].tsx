...
const NAV_DATA = path.join(process.env.NAV_DATA_DIRNAME, NAV_DATA_PREFIX + '-nav-data.json');
...
export default function PluginFrameworkLayout(props) {
    ...
    modifiedProps.navData = modifiedProps.navData.concat(otherPluginsData.filter(({ href }) => !href?.includes(BASE_ROUTE)));
    return (<>
      <DocsPage additionalComponents={{ ProviderTable }} baseRoute={BASE_ROUTE} product={PRODUCT} projectName={PROJECT_NAME} staticProps={modifiedProps} showVersionSelect/>
    </>);
}
const { getStaticPaths, getStaticProps } = getStaticGenerationFunctions(process.env.IS_CONTENT_PREVIEW &&
